...
BASE_DIR = SCRIPT_DIR.parent  # 仓库根目录

# 功能包与短导入（db / poem_admin …）
for _p in (
    SCRIPT_DIR,
    BASE_DIR / "账单",
    BASE_DIR / "账单" / "bills",
...
app = Flask(
    ...
    static_folder=str(SCRIPT_DIR / "site" / "static"),
)
...
SAMPLE_PATH = TOOLS_DIR / "Transfer Dock_Text_20260318112028.txt"
...
def _migrate_single_level_categories():
    ...
    try:
        data_dir = Path(os.environ.get("BILL_DATA_DIR") or (BASE_DIR / "账单" / "数据"))
...
def poem_admin_page():
    ...
    if not _is_poem_admin():
        return render_template("poems/admin.html", is_admin=False, poems=[])

    poems = sort_poems_desc(load_poems())
    return render_template("poems/admin.html", is_admin=True, poems=poems)
...
def poem_create_page():
    ...
    if guard:
        return guard
    return render_template("poems/create_hub.html")
...
def poem_create_form_page():
    ...
    return render_template("poems/edit.html", form=form, poem_id=None)
...
def poem_edit_page(poem_id: int):
    ...
    return render_template("poems/edit.html", form=form, poem_id=poem_id)
