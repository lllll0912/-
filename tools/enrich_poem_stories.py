...
for _p in (ROOT / "modules" / "poems", ROOT / "modules" / "bills"):
...
def main() -> int:
    ...
    parser.add_argument("--llm", action="store_true", help="用大模型润色 life_state/poem_mood/why_write 等")
    parser.add_argument("--skip-search", action="store_true", help="不重新检索 API，仅基于已有 stories 调大模型")
    ...
    if args.llm:
        ...
        if not llm_configured():
            parser.error("已指定 --llm，但未配置 POEM_LLM_API_KEY")
        use_llm = True
    ...
    _safe_print(f"todo {len(targets)} / {len(items)} | llm={'on' if use_llm else 'off'}")
    done = 0
    hit = 0
    llm_ok = 0
    for poem_id, content in targets:
        ...
        try:
            meta = None
            if args.skip_search:
                story = dict(existing or {})
                if not story:
                    story = enhance_without_meta(content, {})
                status = "reuse"
            else:
            ...
            if use_llm:
                from story_llm import enrich_story_with_llm

                story = enrich_story_with_llm(content, story, meta)
                llm_ok += 1
                status += "+llm"
    ...
    build_poems_json()
    _safe_print(f"done {done} hit {hit} llm {llm_ok}, updated stories.json / poems.json")
    return 0
