...
def get_rules_path() -> str:
    ...
    - 本机：账单/数据/category_rules.json
    """
    data_dir = (os.environ.get("BILL_DATA_DIR") or "").strip()
    if data_dir:
        return os.path.join(data_dir, "category_rules.json")
    feature_data = os.path.abspath(os.path.join(BASE_DIR, "..", "数据"))
    return os.path.join(feature_data, "category_rules.json")
...
def ensure_rules_file() -> str:
    ...
    backup_dir = os.path.join(data_dir, "backup") if data_dir else os.path.join(root, "脚本", "备份")
