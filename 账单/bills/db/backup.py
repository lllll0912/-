...
def get_backup_dir() -> str:
    """本机用 脚本/备份；Fly 用 Volume 上 /data/backup（持久）。"""
    data_dir = (os.environ.get("BILL_DATA_DIR") or "").strip()
    if data_dir:
        return str(Path(data_dir) / "backup")
    return str(Path(__file__).resolve().parents[3] / "脚本" / "备份")
...
def _data_root() -> Path:
    ...
    return Path(__file__).resolve().parents[3] / "账单" / "数据"
...
def _poem_root() -> Path:
    ...
    return Path(__file__).resolve().parents[3] / "诗词" / "数据"
