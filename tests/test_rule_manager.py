...
class TestRuleManager(unittest.TestCase):
    def test_infer_known(self):
        ...
        self.assertTrue(isinstance(l2, str) and len(l2) > 0)
    ...
    def test_grouped_options(self):
        ...
        for g in groups:
            self.assertIn("l1", g)
            self.assertIn("l2s", g)
            self.assertIsInstance(g["l2s"], list)
